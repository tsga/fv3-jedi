...
void IOStructuredGrid::read(State & x, const eckit::LocalConfiguration &  fileionames,
  ...
  // 1. Create a distributed model fieldset to determine the variables and structure required
  atlas::FieldSet fieldsCubeSphere;
  x.toFieldSet(fieldsCubeSphere);

  // 2. Set up the target structured grid FieldSet on the writeFunctionSpace_
  atlas::FieldSet fieldsGeographic;
  for (const auto & fieldCubeSphere : fieldsCubeSphere) {
    atlas::Field fieldGeographic = writeFunctionSpace_->createField<double>(
        atlas::option::name(fieldCubeSphere.name()) | 
        atlas::option::levels(fieldCubeSphere.levels()));
    fieldsGeographic.add(fieldGeographic);
  }

  // 3. Determine the input filename (e.g., from params_ or fileionames configuration)
  // Adjust this line depending on how your parameters or file configurations define the input path
  std::string filename = params_.filename.value(); 

  // 4. Read the data from disk on Rank 0
  if (geom_.getComm().rank() == 0) {
    this->readStructuredFields(filename, fieldsGeographic, x.validTime(), fileionames, fileioscaling);
  }

  // 5. [Optional] Handle vertical remapping if doVerticalRemapping is enabled
  if (params_.doVerticalRemapping.value()) {
    ASSERT(params_.orographyFilename.value() != boost::none);
    // Read the structured-grid orography and apply inverse vertical remapping here if needed
  }

  // 6. Interpolate from the structured grid (fieldsGeographic) back to the model grid (fieldsCubeSphere)
  // NOTE: Since the class's default `interpolator_` goes from Model -> Structured Grid,
  // you will need an inverse/reverse global interpolator configured in the opposite direction 
  // (Structured Grid -> Model Grid) to distribute the data back to all MPI ranks:
  //
  // readInterpolator_->apply(fieldsGeographic, fieldsCubeSphere);

  // 7. Populate the State object with the newly read and interpolated fields
